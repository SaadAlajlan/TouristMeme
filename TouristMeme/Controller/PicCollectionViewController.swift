...
class PicCollectionViewController: UIViewController {
    ...
    @IBAction func performPictureAction(_ sender: UIButton) {
        if sender == Editor {
            let image = fetchedResultsController.object(at: memeIndex)
            let images = image.nsData! as Data
//            MemeEditorViewController.picFromflickr(UIImage(data: images)!)
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "MemeEditorViewController") as! MemeEditorViewController
            vc.image = UIImage(data: images)!
